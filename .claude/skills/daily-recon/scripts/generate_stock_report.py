...
def render(analysis: dict, template: str) -> str:
    ...
    for sig in signals:
        ...
        conf_pct = int(float(sig.get("confidence") or 0) * 100)
  ...
  <small>{tech.get('vcp_stage', '')}</small>
    ...
    # DCF
    if val.get("feasible"):
        upside = val.get("upside_pct") or 0
  ...
  <tr><td>Discount Rate</td><td>{fmt_pct(float(val.get('discount_rate') or 0)*100, sign=False)}</td></tr>
</table>
