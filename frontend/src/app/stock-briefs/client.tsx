...
const FILTER_ACTIVE: Record<StanceFilter, string> = {
  all:        "border-foreground/30 bg-foreground/5 text-foreground",
...
export function StockBriefsClient({ dates, allData }: Props) {
  ...
  const reportOpen = selectedAnalysis !== null;
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-5 items-start">
        ...
        <div
          ...
          style={reportOpen ? { maxHeight: "calc(100vh - 260px)" } : {}}
        >
          {filtered.length === 0 ? (
            ...
            filtered.map((a) => (
              <StockCard
                ...
                compact={reportOpen}
                onClick={() => setSelectedTicker((prev) => (prev === a.ticker ? null : a.ticker))}
              />
            ))
          )}
        </div>
        ...
        {/* Report panel */}
        {reportOpen && selectedAnalysis && (
          <div className="flex-1 min-w-0">
            {/* Panel header */}
            <div className="flex items-center justify-between mb-3 rounded-lg border bg-card px-4 py-2">
              <div className="flex items-center gap-2">
                <span className="font-mono font-bold">{selectedAnalysis.ticker}</span>
            ...
            {/* Inline report — scrollable */}
            <div
              className="overflow-y-auto"
              style={{ maxHeight: "calc(100vh - 280px)" }}
            >
              <StockReport analysis={selectedAnalysis} />
            </div>
          </div>
        )}
      </div>
