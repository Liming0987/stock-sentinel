"""Elliott Wave + Fibonacci: trade W4 pullbacks inside a 5-wave impulse.
...
Fibonacci confluence:
  ...
  - Target is standard EW projection (W5 ≈ W1; extended to 1.618×W1 when W3 was strong)
...
class ElliottFibStrategy(BaseStrategy):
    ...
    description = (
        ...
        "Targets W5 Fibonacci projection; stops at Elliott invalidation level."
    )
    max_positions = 2
    ...
    W3_MIN_PCT     = 0.05   # W3 must be a ≥ 5 % move to be worth trading
    MAX_ATR_PCT    = 0.07   # skip if daily ATR > 7% of price (hyper-volatile noise)

    _W4_CONFIDENCE = {0.236: 0.82, 0.382: 0.76, 0.500: 0.68}

    def _find_pivots(self, df) -> List[Tuple[int, float, str]]:
        """
    ...
    def _w4_signal(self, pivots, last_price, atr, ind) -> Optional[Signal]:
        """
        ...
        for i in range(len(pivots) - 4, -1, -1):   # newest → oldest
            ...
            if p2[1] <= p0[1]:                       # W2 fully erased W1
                continue
            if w2 / w1 > self.W2_MAX_RETRACE:
                continue
            if w3 < self.W3_MIN_RATIO * w1:          # W3 shortest → invalid count
                continue
            if w3 / p3[1] < self.W3_MIN_PCT:         # W3 too small to trade
                continue
            ...
            if not (0.10 <= w4_retrace <= self.W4_MAX_RETRACE):
                continue

            # ── Fib confluence ────────────────────────────────────────────────
            hit = None
            for r in self.W4_LEVELS:
                level = p3[1] - r * w3
                if abs(last_price - level) / level <= self.FIB_TOL:
                    hit = (r, level)
                    break
            if hit is None:
                continue
            ...
            conf = self._W4_CONFIDENCE[ratio]
            if w3 >= 1.618 * w1:
    ...
    def evaluate(self, ticker: str, context: Dict) -> Signal:
        ...
        if df is None or len(df) < self.LOOKBACK // 3:
            return Signal.hold()
        ...
        if last_price > 0 and atr / last_price > self.MAX_ATR_PCT:
            return Signal.hold()
        ...
        sig = self._w4_signal(pivots, last_price, atr, ind)
        if sig is not None:
            return sig
