...
_BREAKOUT_BUFFER = 1.01  # price must clear the pivot by ≥1% (matches detect_vcp's "breaking_out")
...
class VCPStrategy(BaseStrategy):
    ...
    def evaluate(self, ticker: str, context: Dict) -> Signal:
        ...
        # Never add to / re-enter an open position. Exits are handled by the base
        # should_close() using the stop_loss and target set at entry — the run loop
        # only calls evaluate() when there is no open position, so a sell branch here
        # would be dead code.
        if current_position:
            return Signal.hold()
        ...
        # Use the live price — indicators.last_price is overridden with the real-time
        # Alpaca quote during market hours; fall back to the last daily close.
        last_price = float(ind.get("last_price") or df["Close"].iloc[-1])
        pivot = float(vcp["pivot"])
        vol_ratio = float(ind.get("volume_ratio") or 0)
        contractions = vcp["contractions"]

        # Require a decisive break above the pivot (≥1%), not a razor-thin touch.
        breaking_out = last_price > pivot * _BREAKOUT_BUFFER and vol_ratio >= _MIN_VOL_RATIO
        ...
        # Note: the strategy runner applies apply_fundamental_modifier() to every
        # buy signal, so we must NOT apply it here too (would double-count it).
        return signal
