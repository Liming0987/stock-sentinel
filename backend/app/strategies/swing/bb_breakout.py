...
Adjustments (2026-06-03 backtest):
  ...
  - Stop: max(BB_middle, entry - 1.5×ATR) — caps the worst-case loss at 1.5×ATR. On
    volatile tickers (SOFI ATR=5.6% of price) the bands are wide, so the middle band
    sits far below entry and a pure BB-middle stop risked too much; the ATR floor
    (the higher of the two = tighter stop) limits the loss.
"""
...
class BBBreakoutStrategy(BaseStrategy):
    ...
    MIN_VOL_RATIO = 2.0
    TARGET_ATR_MULT = 4.0
    STOP_ATR_MULT = 1.5     # loss cap — limits worst-case loss to 1.5×ATR when the BB middle is far below entry
