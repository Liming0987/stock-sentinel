...
class OpeningRangeBreakoutStrategy(BaseStrategy):
    ...
    def evaluate(self, ticker: str, context: Dict) -> Signal:
        ...
        # ORB sizes its stop/target from the opening range, not ATR — so intraday_atr
        # isn't needed here (unlike VWAP Cross, which uses it for the stop).
        if not all(v is not None for v in (orb_high, orb_low, current_price)):
            return Signal.hold()
