...
class VWAPCrossStrategy(BaseStrategy):
    ...
    def evaluate(self, ticker: str, context: Dict) -> Signal:
        ...
        prev_bar_vwap = intraday.get("prev_bar_vwap")
        ...
        # Cross: previous bar closed below its own VWAP; current price is above today's VWAP.
        # Using per-bar VWAP values avoids comparing a prior-bar close to a current-bar
        # cumulative VWAP, which would shift the crossover threshold.
        prev_vwap_ref = prev_bar_vwap if prev_bar_vwap is not None else vwap
        crossed_above = prev_close < prev_vwap_ref and current_price >= vwap
    ...
    def should_close(self, trade, context: Dict) -> Optional[str]:
        ...
        # Wall-clock EOD safety net: close any open VWAP position after 3:45 ET
        # regardless of intraday context availability (e.g. when run_eod
        ...
        # Bar 75 = 3:45 PM (75 × 5 min after 9:30), consistent with ORB and the
        # wall-clock cutoff above.
        if intraday.get("bars_elapsed", 0) >= 75:
            return "end_of_day"

        current_price = intraday.get("current_price")
        vwap = intraday.get("vwap")
        # Use the yfinance bar close for the exit comparison to avoid whipsawing
        # out of a position on a live-quote spike against a delayed VWAP level.
        bar_close = intraday.get("prev_bar_close")
        exit_price = bar_close if bar_close is not None else current_price
        if exit_price and vwap and exit_price < vwap * 0.999:
